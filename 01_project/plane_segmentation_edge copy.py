...
def compute_contours(clusters, method='concave', alpha=0.1):
    ...
    返回:
        contours: Shapely Polygon或MultiPolygon对象列表
    """
    contours = []
...
def visualize_contours(points_2d, contours, title="Projection Contours"):
    """
    可视化投影点和轮廓
    参数:
        points_2d: 所有2D点坐标
    ...
    plt.scatter(points_2d[:, 0], points_2d[:, 1], c='blue', s=1, label='Points')
    
    for i, contour in enumerate(contours):
        if contour.geom_type == 'Polygon':
            x, y = contour.exterior.xy
            plt.plot(x, y, '-', lw=2, label=f'Contour {i+1}')
...
# 2. 对投影后的点云进行聚类
clusters = cluster_points(projected_2d, eps=0.15)  # 可调整eps参数

# 3. 计算每个簇的轮廓
contours = compute_contours(clusters, method='concave', alpha=0.05)

# 4. 可选: 可视化结果
visualize_contours(projected_2d, contours, title="Separated Contours")

# 5. 使用轮廓结果
for i, contour in enumerate(contours):
    print(f"轮廓 {i+1} 面积: {contour.area}")
